...
abstract class GitOperation(fileDir: File, internal val callingActivity: Activity) {
    ...
    private fun withAndroidKeystoreKey(username: String): GitOperation {
        val sessionFactory = SshjSessionFactory(username, SshAuthData.AndroidKeystoreKey("ssh_key"), hostKeyFile)
        SshSessionFactory.setInstance(sessionFactory)
        this.provider = null
        return this
    }
    ...
    fun executeAfterAuthentication(
        ...
        when (connectionMode) {
            ConnectionMode.SshKey -> {
                ...
                } else if (sshKeyFile.readText() == "keystore") {
                    withAndroidKeystoreKey(username).execute()
                } else {
